...
train_data = torch.load('feature.pt').to(torch.float).to(device)
...
for x in range(32):
    ...
    with torch.no_grad():
        ...
        train_accuracy = accuracy_score(y_true_train, [x[0] for x in y_pred_train])
        test_accuracy = accuracy_score(y_true_test, [x[0] for x in y_pred_test])
