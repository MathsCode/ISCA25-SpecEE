...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.predictors = [torch.load('/home/xujiaming/xujiaming/train_machine/PJY/ISCA/ASPLOS-arranged/AutoEE/Classifier/old_predictors/llama/base/7B/mixed/model'+str(layer_idx)+'.pth').to(torch.float16) for layer_idx in range(config.num_hidden_layers)]
    ...
    def forward(
        ...
        if len(exit_layer_id_list) >= 5:
            for x in exit_layer_id_list[-5:]:
                for y in [-2,0,2]:
                    this_layer = x+y-1
                    if this_layer >= 0 and this_layer<31 and not this_layer in dynamic_layers:
        ...
        for idx,decoder_layer in enumerate(self.layers):
            ...
            #             ), token
            # if input_ids[0][0]==596:
            #     breakpoint()
            # import random
            # layer_selected = random.sample(range(32),24)
            if not init and idx <= len(self.layers):
                if idx not in layer_selected:       # 7B
                    continue
                hidden_states_tmp = self.norm(hidden_states)
                draft_logits = F.linear(hidden_states_tmp, draft_lm_head_weight)
                draft_prob = F.softmax(draft_logits, dim=-1)
